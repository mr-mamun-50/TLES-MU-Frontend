...
export default function SuppleMarks() {
  ...
  const updateOrDeleteExamMarks = () => {
    setLoading(true)
    axios.put(`/api/user/obtained-supple-marks/${editableMarks.id}`, editableMarks).then(res => {
      if (res.status === 200) {
        setSuccess(res.data.message)
        setTimeout(() => { setSuccess('') }, 5000)
        getStudents()
  ...
  return (
    <Box className={`container`}>
      <Box className={`card my-2`}>
        ...
        <Box className='card-header pb-0 d-flex justify-content-between align-items-end' sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          <Box className="text-end">
            ...
            {role === 'user' &&
              <Link to={`/supplementary-exams/marks/import/${exam.id}`} state={{ exam: exam, question_sets: exam.question_sets, students: students }}
        ...
        {loading ? <Box className="text-center"><span className='spinner-border my-5'></span></Box> :
              ...
              <Table className='table-bordered table-sm border-grey'>
                ...
                <TableBody>
                  {students.map((student, student_index) => {
                    ...
                    const totalObtainedMarks = student.obtained_supple_marks.reduce((acc, obtained_marks) => acc + obtained_marks.marks, 0);
                    ...
                    return (
                      <TableRow key={student_index}>
                        ...
                        {question_sets.map((question_set) => (
                          question_set.questions.map((question, question_index) => {
                            const obtainedMark = student.obtained_supple_marks.find(obtained_marks => obtained_marks.question_id === question.id);
          ...
          <Box>
            ...
            <Box className='scroll' sx={{ height: 300, width: 300, overflowY: 'scroll' }}>
              ...
              {question_sets[activeStep]?.questions.map((question, question_index) => {
                const obtainedMark = inputStudent.obtained_supple_marks?.find(obtained_marks => obtained_marks.question_id === question.id);
      ...
      {/* edit marks modal */}
      <ModalDialog
        title={`${editableMarks.student_id} (${editableMarks.qstn_no})`}
        content={
          <Box>
        ...
        onConfirm={updateOrDeleteExamMarks}
